                    ...
                    if field.get('type') == 'signature' or 'Signature' in field.get('name', ''):
                        ...
                        if ('Applicant' in field_name or field['pdf_field_name'] == 'signature3') and user2_data.get('applicant_signature'):
                            ...
                            if signature_value.startswith('data:image/'):
                                ...
                                print(f"🖋️  Applied Applicant signature (image) to field '{field['name']}' (pdf_field: '{field['pdf_field_name']}'): [Digital Signature]")
                            else:
                                # For typed signatures, use the text directly
                                field['value'] = signature_value
                                field['is_image_signature'] = False
                                print(f"🖋️  Applied Applicant signature (text) to field '{field['name']}' (pdf_field: '{field['pdf_field_name']}'): '{signature_value[:20]}...'")
                        ...
                        # Map Property Owner Signature field  
                            ...
                            if signature_value.startswith('data:image/'):
                                ...
                                field['is_image_signature'] = True
                                field['image_data'] = signature_value
                                print(f"🖋️  Applied Property Owner signature (image) to field '{field['name']}' (pdf_field: '{field['pdf_field_name']}'): [Digital Signature]")
                            else:
                                # For typed signatures, use the text directly
                                field['value'] = signature_value
                                field['is_image_signature'] = False
                                print(f"🖋️  Applied Property Owner signature (text) to field '{field['name']}' (pdf_field: '{field['pdf_field_name']}'): '{signature_value[:20]}...'")
