...
class PDFProcessor:
    ...
    def fill_pdf_with_pymupdf(self, pdf_path: str, document: Dict[str, Any], output_path: str) -> bool:
        ...
        try:
                        ...
                        try:
                            ...
                            if widget_type == 'radio':
                                ...
                                if str(field_value).lower() in ['yes', 'true', '1', 'on']:
                                    # For radio buttons, we need to find the correct export value
                                    try:
                                        # First discover what the "on" value should be
                                        widget.field_value = True
                                        widget.update()
                                        actual_on_value = widget.field_value
                                        print(f"🔍 Radio '{field_name}' discovered on value: '{actual_on_value}'")
                                        
                                        # Reset and set to the proper value
                                        widget.field_value = False
                                        widget.update()
                                        
                                        # Now set to the correct "on" value
                                        if actual_on_value and str(actual_on_value) not in ['False', 'Off', '']:
                                            widget.field_value = actual_on_value
                                            widget.update()
                                            final_value = widget.field_value
                                            if final_value == actual_on_value:
                                                print(f"✅ Filled radio field '{field_name}' with export value: '{actual_on_value}'")
                                                filled_count += 1
                                            else:
                                                # Fallback to True
                                                widget.field_value = True
                                                widget.update()
                                                print(f"✅ Filled radio field '{field_name}' with True (fallback)")
                                                filled_count += 1
                                        else:
                                            # Standard boolean approach
                                            widget.field_value = True
                                            widget.update()
                                            print(f"✅ Filled radio field '{field_name}' with True (standard)")
                                            filled_count += 1
                                            
                                    except Exception as e:
                                        print(f"⚠️  Error setting radio '{field_name}': {e}")
                                        # Last resort
                                        try:
                                            widget.field_value = True
                                            widget.update()
                                            filled_count += 1
                                            print(f"⚠️  Fallback: Set radio '{field_name}' to True")
                                        except:
                                            print(f"❌ Complete failure for radio '{field_name}'")
                                else:
                                ...
                                if str(field_value).lower() in ['true', 'yes', '1', 'checked']:
                                    # Find the proper "on" state for this checkbox
                                    try:
                                        # Get all possible values for this checkbox
                                        current_value = widget.field_value
                                        print(f"🔍 Checkbox '{field_name}' current value: '{current_value}'")
                                        
                                        # For PyMuPDF checkboxes, we need to find the "checked" export value
                                        # Common patterns: 'Yes', 'On', 'True', or values like 'Yes_xyz'
                                        
                                        # Method 1: Use the actual export value from the PDF
                                        # First, try to set to any value to see what the current export value is
                                        widget.field_value = True
                                        widget.update()
                                        actual_value = widget.field_value
                                        print(f"🔍 After setting True, checkbox value is: '{actual_value}'")
                                        
                                        # Reset and try to use the actual export value
                                        widget.field_value = False
                                        widget.update()
                                        
                                        success = False
                                        
                                        # Method 1a: Use the value we just discovered
                                        if actual_value and str(actual_value) not in ['False', 'Off', '']:
                                            try:
                                                widget.field_value = actual_value
                                                widget.update()
                                                final_value = widget.field_value
                                                if final_value == actual_value:
                                                    print(f"✅ Successfully set checkbox '{field_name}' to discovered value: '{actual_value}'")
                                                    success = True
                                            except Exception as e1:
                                                print(f"⚠️  Method 1a failed for '{field_name}': {e1}")
                                        
                                        # Method 2: Try extracting base from export values like 'Yes_xyz'
                                        if not success and actual_value and '_' in str(actual_value):
                                            try:
                                                base_value = str(actual_value).split('_')[0]
                                                widget.field_value = base_value
                                                widget.update()
                                                if widget.field_value == base_value:
                                                    print(f"✅ Set checkbox '{field_name}' using base export value: '{base_value}'")
                                                    success = True
                                                else:
                                                    # Try the full value again
                                                    widget.field_value = actual_value
                                                    widget.update()
                                                    print(f"✅ Set checkbox '{field_name}' using full export value: '{actual_value}'")
                                                    success = True
                                            except Exception as e2:
                                                print(f"⚠️  Method 2 failed for '{field_name}': {e2}")
                                        
                                        # Method 3: Try common checkbox values
                                        if not success:
                                            checkbox_on_values = ['Yes', 'On', 'True', '1', 'Checked']
                                            for on_value in checkbox_on_values:
                                                try:
                                                    widget.field_value = on_value
                                                    widget.update()
                                                    if widget.field_value == on_value:
                                                        print(f"✅ Successfully set checkbox '{field_name}' to standard value: '{on_value}'")
                                                        success = True
                                                        break
                                                except:
                                                    continue
                                        
                                        # Method 4: Try setting as boolean True (final fallback)
                                        if not success:
                                            try:
                                                widget.field_value = True
                                                widget.update()
                                                print(f"✅ Set checkbox '{field_name}' to boolean True (fallback)")
                                                success = True
                                            except Exception as e3:
                                                print(f"⚠️  Method 4 failed for '{field_name}': {e3}")
                                        
                                        if success:
                                            filled_count += 1
                                            print(f"✅ Filled checkbox field '{field_name}' with True (checked)")
                                        else:
                                            print(f"❌ Failed to check checkbox '{field_name}' - all methods failed")
                                            
                                    except Exception as e:
                                        print(f"⚠️  Error setting checkbox '{field_name}': {e}")
                                        # Last resort fallback
                                        try:
                                            widget.field_value = True
                                            widget.update()
                                            filled_count += 1
                                            print(f"⚠️  Fallback: Set checkbox '{field_name}' to True")
                                        except:
                                            print(f"❌ Complete failure for checkbox '{field_name}'")
                                else:
                                ...
                                # Also set the text appearance for better visibility
                                try:
                                    # Force the appearance stream update
                                    widget.update()
                                    # Make sure field is not readonly for the fill operation
                                    original_flags = widget.field_flags
                                    widget.field_flags = original_flags & ~1  # Remove readonly flag temporarily
                                    widget.field_value = str(field_value)
                                    widget.update()
                                    widget.field_flags = original_flags  # Restore original flags
                                except:
                                    pass
            ...
            # Create a flattened version by adding text overlays
            print("🔧 Creating flattened PDF with text overlays...")
            try:
                flattened_count = 0
                for page_num in range(len(doc)):
                    page = doc[page_num]
                    ...
                    for widget in widgets:
                        field_name = widget.field_name
                        if field_name and field_name in field_mapping and field_name not in signature_fields:
                            field_value = field_mapping[field_name]
                            widget_type = self.get_widget_type(widget)
                            
                            # Skip radio buttons and checkboxes (they have visual indicators)
                            if widget_type in ['radio', 'checkbox']:
                                continue
                                
                            # Add text overlay for visible text fields
                            if field_value and widget_type in ['text']:
                                rect = widget.rect
                                
                                # Insert text overlay with white background
                                page.insert_text(
                                    (rect.x0 + 2, rect.y0 + (rect.height * 0.7)),
                                    str(field_value),
                ...
                print(f"✅ Added {flattened_count} text overlays for better visibility")
                            
            except Exception as e:
                print(f"⚠️  Warning: Could not create text overlays: {e}")
            
            # Regenerate appearance streams for all filled fields
            print("🔄 Regenerating appearance streams...")
            try:
                ...
                for page_num in range(len(doc)):
                    ...
                    for widget in widgets:
                        if widget.field_name and widget.field_name in field_mapping:
                            try:
                ...
                print(f"✅ Updated appearance streams for {appearances_updated} widgets")
            except Exception as e:
                print(f"⚠️  Warning: Could not update appearance streams: {e}")
            
            # Save the filled PDF
            print(f"💾 Saving PDF to: {output_path}")
            # Save with incremental=False to ensure a complete rewrite and clean=True to optimize
            ...
            if os.path.exists(output_path):
                ...
                print(f"💾 PDF saved successfully: {saved_size} bytes")
                
                # Quick validation of saved PDF
                try:
                    with open(output_path, 'rb') as f:
                        header = f.read(5)
                        if header.startswith(b'%PDF'):
                            print("✅ Saved PDF has valid header")
                        else:
                            print(f"❌ Saved PDF has invalid header: {header}")
                            return False
                except Exception as e:
                    print(f"❌ Error validating saved PDF: {e}")
                    return False
            else:
